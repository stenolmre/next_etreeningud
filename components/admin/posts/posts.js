            ...
            {posts &&
              posts
                ...
                .map((el) => (
                  <div
                    key={el._id}
                    className="admin_row admin_list admin_posts_list"
                  >
                    ...
                    <Anchor
                      id={el._id}
                      name={new Date(el.createdAt).toLocaleDateString()}
                    />
                    <p>
                      {analytics &&
                        analytics.filter((_el) => _el.id === el._id).length}
                    </p>
                    <div>
                      <Link href={`/private/admin/editpost?id=${el._id}`}>
                      ...
                      <i
                        ...
                        onClick={async () => {
                          if (
                            confirm(
                              "Kas sa oled kindel, et soovid postituse kustutada?",
                            )
                          ) {
                ...
                .slice(0, numOfLoadedPosts)}
            {numOfLoadedPosts < 10 || numOfLoadedPosts >= numOfPosts ? null : (
              <p
                onClick={() =>
                  router.push(
                    `/private/admin/posts?num=${numOfLoadedPosts + 10}`,
                    undefined,
                    { shallow: true },
                  )
                }
...
const Anchor = ({ id, name, style }) => {
  return (
    <Link href={`/posts/${id}?name=${name.replaceAll(" ", "-")}`} style={style}>
      {name}
    </Link>
  );
};
