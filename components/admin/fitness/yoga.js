          ...
          <Fragment>
              ...
              fitness
                ...
                .map((fit) => (
                  <div key={fit._id} className="admin_row admin_list">
                    ...
                    <div>
                      <Link href={`/private/admin/edityoga?id=${fit._id}`}>
                        <a>
                      ...
                      <i
                        ...
                        onClick={async () => {
                          if (
                            confirm(
                              "Kas sa oled kindel, et soovid joogatunni kustutada?",
                            )
                          ) {
            ...
            numOfLoadedWorkouts >= numOfWorkouts ? null : (
              <p
                onClick={() =>
                  router.push(
                    `/private/admin/yoga?num=${numOfLoadedWorkouts + 10}`,
                    undefined,
                    { shallow: true },
                  )
                }
...
const Anchor = ({ id, name }) => {
  return (
    <Link href={`/yoga/${id}?name=${name.toLowerCase().replaceAll(" ", "-")}`}>
      {name}
    </Link>
  );
};
