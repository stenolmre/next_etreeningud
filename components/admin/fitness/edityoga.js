...
export default function EditYoga() {
  ...
  const [yogaData, setYogaData] = useState({ image: '', name: '', category: 'jooga', equipment: 'joogamatt', length: 0, intro: '', video: '' })

  const onChange = e => setYogaData({ ...yogaData, [e.target.name]: e.target.value })

  const addNewWorkout = async () => {
    setProcessing(true)
    await updateWorkout(dispatchFit, query.id, yogaData, () => {
      setSuccess(true)
      setProcessing(false)
    }, () => {
      setError(true)
  ...
  return <Layout name={workout && workout.name}>
    <div className="admin_add_workout admin_page">
      ...
      <select name="category" id="category" value={yogaData.category} onChange={e => setYogaData({ ...yogaData, category: e.target.value })}>
      ...
      <Link href="/private/admin/yoga"><a className="admin_edit_workout_go_back">Tagasi</a></Link>
