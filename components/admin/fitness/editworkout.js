...
export default function EditWorkout() {
  ...
  const addToArray = (state, arr) => {
    if (state.gif !== "") {
      const exercise_name =
        fit_exercises &&
        fit_exercises.filter((el) => state.gif === el.fit_exercises_gif)[0]
          .fit_exercises_name;
  ...
  useEffect(() => {
    setWorkoutData((workoutData) => ({
      ...
      warmup: !workout || !workout.warmup ? [] : workout.warmup,
      workout: !workout || !workout.workout ? [] : workout.workout,
      cooldown: !workout || !workout.cooldown ? [] : workout.cooldown,
    }));
  }, [workout]);

  return (
    <Layout name="Muuda Treeningut">
      <div className="admin_add_workout admin_page">
          ...
          {fit_images &&
            fit_images.map((el) => (
              <img
                ...
                src={el}
                alt={el}
                onClick={() => setWorkoutData({ ...workoutData, image: el })}
                style={
                  workoutData.image === el
        ...
        <select
          ...
          onChange={(e) =>
            setWorkoutData({ ...workoutData, category: e.target.value })
          }
        >
          <option value="" disabled={workoutData.category !== ""}>
            ..
          </option>
          {fit_categories &&
            fit_categories
        ...
        <label>
          Vahendid <span className="form_required">*</span>
        </label>
        <select
          name="equipment"
          id="equipment"
        ...
        >
          <option value="" disabled={workoutData.equipment !== ""}>
            ..
          </option>
          {fit_equipment &&
            fit_equipment.map((el, i) => (
              <option key={i} value={el}>
                {el}
              </option>
            ))}
        ...
        <textarea
          name="intro"
          value={workoutData.intro}
          onChange={onChange}
          maxLength="105"
        />
        <h3>Soojendus</h3>
        <Header />
        {workoutData.warmup.length < 1 && (
          <p style={{ fontSize: ".75rem" }}>Lisa soojendusharjutused.</p>
        )}
        {workoutData.warmup.map((el, i) => (
          <div key={i} className="admin_row admin_add_workout_add_exercise">
            <p>{el.name}</p>
            <p>{el.gif}</p>
            <input
              name="reps"
            ...
            <i
              ...
              onClick={() => {
                const newArray = workoutData.warmup.filter(
                  (x) => x.gif !== el.gif,
                );
        ...
        <AddExercise
          ...
          state={newWarmup}
          name="warmup"
          condition={workoutData.equipment}
          fitExercises={fit_exercises}
          action={() => addToArray(newWarmup, workoutData.warmup)}
        />
        <h3>Treening</h3>
        <Header />
        {workoutData.workout.length < 1 && (
          <p style={{ fontSize: ".75rem" }}>Lisa harjutused.</p>
        )}
        {workoutData.workout.map((el, i) => (
          <div key={i} className="admin_row admin_add_workout_add_exercise">
            <p>{el.name}</p>
            <p>{el.gif}</p>
            <input
            ...
            <i
              className="fas fa-times"
              onClick={() => {
                const newArray = workoutData.workout.filter(
                  (x) => x.gif !== el.gif,
                );
        ...
        <AddExercise
          ...
          name="workout"
          fitExercises={fit_exercises}
          action={() => addToArray(newWorkout, workoutData.workout)}
        />
        <h3>Cooldown</h3>
        <Header />
        {workoutData.cooldown.length < 1 && (
          <p style={{ fontSize: ".75rem" }}>Lisa lõdvestusharjutused.</p>
        )}
        {workoutData.cooldown.map((el, i) => (
          <div key={i} className="admin_row admin_add_workout_add_exercise">
            <p>{el.name}</p>
            <p>{el.gif}</p>
            <input
            ...
            <i
              className="fas fa-times"
              onClick={() => {
                const newArray = workoutData.cooldown.filter(
                  (x) => x.gif !== el.gif,
                );
        ...
        <AddExercise
          ...
          name="cooldown"
          fitExercises={fit_exercises}
          action={() => addToArray(newCooldown, workoutData.cooldown)}
        />
        <button
          disabled={processing}
        ...
        <Link
          href="/private/admin/fitness"
          className="admin_edit_workout_go_back"
        >
          Tagasi
        </Link>
        {success && (
          <p style={{ color: "rgba(0, 112, 243, 1)", marginLeft: "-10px" }}>
            Salvestatud.
          </p>
        )}
        {error && (
...
const Header = () => (
  <div className="admin_row_header admin_add_workout_add_exercise_header">
    <p>Nimi</p>
    <p>Gif</p>
...
const AddExercise = ({ set, state, name, fitExercises, action }) => (
  <div className="admin_add_workout_exercises_list">
    <select
      name={name}
      id={name}
      onChange={(e) => set({ ...state, gif: e.target.value })}
    >
      <option value="">..</option>
      {fitExercises &&
        fitExercises.map((el, i) => (
          <option key={i} value={el.fit_exercises_gif}>
            {el.fit_exercises_name}
          </option>
        ))}
    </select>
