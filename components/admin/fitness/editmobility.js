...
export default function EditMobility() {
  ...
  const onChange = (e) =>
    setMobilityData({ ...mobilityData, [e.target.name]: e.target.value });
  ...
  return (
    <Layout name="Muuda Liikuvustreeningut">
      <div className="admin_add_workout admin_page">
          ...
          {fit_images &&
            fit_images.map((el) => (
              <img
                ...
                src={el}
                alt={el}
                onClick={() => setMobilityData({ ...mobilityData, image: el })}
                style={
                  mobilityData.image === el
        ...
        <select
          ...
          id="category"
          onChange={(e) =>
            setMobilityData({ ...mobilityData, category: e.target.value })
          }
        >
          <option value="" disabled={mobilityData.category !== ""}>
            ..
          </option>
          {fit_categories &&
            fit_categories
              .filter((el) => el === "mobility")
              .map((el, i) => (
