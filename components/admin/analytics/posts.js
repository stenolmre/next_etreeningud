...
const Fitness = ({ analytics }) => {
  ...
  const numOfPosts =
    analytics && analytics.filter((el) => el.category === "blog").length;
      ...
      {loading ? (
        ...
        <Fragment>
          {analytics &&
            posts &&
            analytics
              .filter((el) => el.category === "blog")
              .map((el) => (
                <Link
                  key={el._id}
                  href={`/posts/${el.id}?name=${findPost(el.id) && findPost(el.id).name.toLowerCase().replaceAll(" ", "-")}`}
                  className="admin_row admin_list admin_row_analytics"
                >
                  <img
                    src={findPost(el.id) && findPost(el.id).image}
                    alt={el.id}
                  />
                  <p>
                    {findPost(el.id)
          ...
          {numOfLoadedPosts < 10 || numOfLoadedPosts >= numOfPosts ? null : (
            <p
              onClick={() =>
                router.push(
                  `/private/admin/analytics?page=posts&num=${numOfLoadedPosts + 10}`,
                  undefined,
                  { shallow: true },
                )
              }
