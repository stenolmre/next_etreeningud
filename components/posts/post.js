...
const Post = ({ post }) => {
  ...
  const { loading } = useWriterState()

  useEffect(() => {
    getWriters(dispatchWriter)
  }, [dispatchWriter])
      ...
      {post && (
        <Fragment>
          ...
          {loading ? (
            ...
            writers &&
            writers
              .filter(el => el.name.toLowerCase() === post.author.toLowerCase())
              .map(writer => <Writer key={writer._id} writer={writer} />)
          )}
          <Ad id={post._id} />
        </Fragment>
      )}
