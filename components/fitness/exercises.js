...
const Exercise = ({ color, heading, state, setState, type, open, close, id }) => {
  return <Fragment>
    <div className="workout_section_heading" id={id}>
      <h3 style={{ color: color }}>{heading}</h3>
      <div onClick={state.length < type.length ? () => setState(Array.from(Object.values(type), x => x._id)) : () => setState([])}>
        <p>{state.length < type.length ? 'ava' : 'sulge'} videod</p>
        <i className={state.length < type.length ? "fas fa-tv" : 'fas fa-tv inset_box_shadow'}/>
      </div>
    </div>
    {
      type.map((x, i) => <div key={x._id} className={!state.includes(x._id) ? 'workout_exercise_closed' : 'workout_exercise'}>
        <div className="workout_exercise_details" onClick={state.includes(x._id) ? () => close(state, setState, x._id) : () => open(state, setState, x._id)}>
          <h4>{x.name}</h4>
          <p>{x.reps}</p>
        </div>
