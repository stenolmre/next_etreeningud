...
const Card = ({ fit }) => {
  return <Link href={fit.category !== 'jooga' ? fit.category === 'mobility' ? `/mobility/${fit._id}?name=${fit.name.replaceAll(' ', '-')}` : `/fitness/${fit._id}?name=${fit.name.replaceAll(' ', '-')}` : `/yoga/${fit._id}?name=${fit.name.replaceAll(' ', '-')}`}><a className="blog_card">
