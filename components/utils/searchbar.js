...
const Searchbar = ({ onChange, placeholder, href, fitness }) => {
  ...
  return (
    <div className={fitness ? "searchbar fitness_searchbar" : "searchbar"}>
      <div>
      ...
      {router.query.search && (
        <p>Leitud tulemused otsingule "{router.query.search}".</p>
      )}
    </div>
  );
};
...
const Category = ({ href, icon, name, all, category }) => {
  ...
  return (
    <Link
      href={all ? href : `/fitness?category=${category}`}
      className="searchbar_category"
    >
      {all ? (
        <i
          className={
            !query.category
              ? `${icon} inset_neumorphism`
              : `${icon} neumorphism`
          }
        />
      ) : (
