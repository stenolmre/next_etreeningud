...
const Form = () => {
  ...
  const [error, setError] = useState({
    state: false,
    email: "Palun sisestage korrektne email.",
    message: "Palun sisestage sõnum.",
  });
  ...
  return (
    <form className="contact_form" onSubmit={onSubmit}>
      ...
      <label>
        Sõnum <span className="form_required">*</span>
      </label>
      <textarea
        name="message"
      ...
      <button disabled={processing} type="submit">
        {processing ? "Saadan.." : "Saada"}
      </button>
      {success && (
        <p className="form_success">
          Sõnum on edukalt saadetud! Aitäh! Vastame Sulle iga hetk.
        </p>
      )}
    </form>
  );
