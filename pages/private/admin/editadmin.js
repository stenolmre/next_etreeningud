...
import EditAdminContent from './../../../components/admin/admins/editadmin'

const EditAdmin = ({ user_token }) => {
  const dispatchAdmin = useAdminDispatch()

  useEffect(() => { getAdmin(dispatchAdmin, user_token) }, [dispatchAdmin, user_token])

  return <Fragment>
    <Head title="Muuda Admini Andmeid"/>
    <EditAdminContent />
  </Fragment>
}

EditAdmin.getInitialProps = async ctx => {
