...
import AddMobilityContent from './../../../components/admin/fitness/addmobility'

const AddMobility = ({ user_token }) => {
  const dispatchAdmin = useAdminDispatch()

  useEffect(() => { getAdmin(dispatchAdmin, user_token) }, [dispatchAdmin, user_token])

  return <Fragment>
    <Head title="Lisa Liikuvustreening"/>
    <AddMobilityContent />
  </Fragment>
}

AddMobility.getInitialProps = async ctx => {
