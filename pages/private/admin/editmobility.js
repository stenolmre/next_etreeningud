...
import EditMobilityContent from './../../../components/admin/fitness/editmobility'

const EditMobility = ({ user_token }) => {
  const dispatchAdmin = useAdminDispatch()

  useEffect(() => { getAdmin(dispatchAdmin, user_token) }, [dispatchAdmin, user_token])

  return <Fragment>
    <Head title="Muuda Liikuvustreeningut"/>
    <EditMobilityContent />
  </Fragment>
}

EditMobility.getInitialProps = async ctx => {
  ...
  try {
    ...
    const { data } = !process.env.NODE_ENV || process.env.NODE_ENV === 'development'
      ...
      : await axios.get('https://et-tau.vercel.app/api/admin/get')
