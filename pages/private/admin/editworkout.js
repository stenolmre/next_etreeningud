...
import EditWorkoutContent from './../../../components/admin/fitness/editworkout'

const EditWorkout = ({ user_token }) => {
  const dispatchAdmin = useAdminDispatch()

  useEffect(() => { getAdmin(dispatchAdmin, user_token) }, [dispatchAdmin, user_token])

  return <Fragment>
    <Head title="Muuda Treeningut"/>
    <EditWorkoutContent />
  </Fragment>
}

EditWorkout.getInitialProps = async ctx => {
